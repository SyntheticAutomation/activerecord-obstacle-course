...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where("user_id = 3").third.items.pluck("items.name")
    # ------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).group("users.name").select("users.name", "count(orders.id) as total_order_count")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).group("users.name").order("users.name DESC").select("users.name", "count(items.id) as total_item_count")
    # ---------------------------------------------------------------
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.all.includes(:order_items, :items)
    # ------------------------------------------------------
